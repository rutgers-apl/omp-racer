...
void __kmp_serialized_parallel(ident_t *loc, kmp_int32 global_tid) {
  ...
  if (ompt_enabled.enabled &&
    ...
    if (ompt_enabled.ompt_callback_parallel_begin) {//omp-whip parallel begin start
      int team_size = 1;
...
#if OMPT_SUPPORT
    if (ompt_enabled.enabled) {
      if (ompt_enabled.ompt_callback_parallel_begin) {//omp-whip parallel begin
