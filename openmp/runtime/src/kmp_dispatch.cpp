...
__kmp_dispatch_init(ident_t *loc, int gtid, enum sched_type schedule, T lb,
  ...
  if (ompt_enabled.ompt_callback_work) {//omp-whip loop begin
    ...
    ompt_callbacks.ompt_callback(ompt_callback_work)(
        ...
        &(task_info->task_data), pr->u.p.tc, OMPT_LOAD_RETURN_ADDRESS(gtid), loc->psource);
  }
#endif
...
#if OMPT_SUPPORT && OMPT_OPTIONAL  // callback defined in OpenMP tr 7 omp-whip
