...
OpenmpRaceDetector::m_isparallel(GraphType curr_step, GraphType meta_access){
  ...
  if(rem_seq < curr_seq){
    if(prev_remote_task != 0 && m_tgraph_nodes[prev_remote_task].m_type == PARALLEL){
      if(m_tgraph_nodes[prev_curr_task].m_type == PARALLEL && m_are_dependent(prev_curr_task, prev_remote_task)){
        ret_val = false;
      }
      else{
        ret_val = true;
    ...
    if(prev_curr_task != 0 && m_tgraph_nodes[prev_curr_task].m_type == PARALLEL){
      if(m_tgraph_nodes[prev_curr_task].m_type == PARALLEL && m_are_dependent(prev_remote_task, prev_curr_task)){
        ret_val = false;
      }
      else{
        ret_val = true;
...
#endif
{
  ...
  GraphType write1 = addr_vec->getWrite1();
  GraphType read1 = addr_vec->getRead1();
  GraphType read2 = addr_vec->getRead2();
  
  //temp
  #if DEBUG
  report[tid] << addr << "," << type << ",cur_w=" << curr_step << ",w=" 
  << write1 << ",r1=" << read1 << ",r2=" << read2 << std::endl;
  #endif
  
  //std::cout << "curr_w=" << curr_step << ", addr_vec = " << addr_vec << ", add_vec->write1 = " << addr_vec->write1 << std::endl;
  if(m_isparallel(curr_step, write1)){
    //report race
    #if LOC_INFO
    m_add_violations(addr, curr_step, type, write1, WRITE, loc);  
    #else
    m_add_violations(addr, curr_step, type, write1, WRITE);  
    #endif
    
  }
  
  if(type == WRITE){
    if(m_isparallel(curr_step, read1)){
      #if LOC_INFO
      m_add_violations(addr, curr_step, WRITE, read1, READ, loc);
      #else
      m_add_violations(addr, curr_step, WRITE, read1, READ);
      #endif
      //report race
    }
    if(m_isparallel(curr_step, read2)){
      #if LOC_INFO
      m_add_violations(addr, curr_step, WRITE, read2, READ, loc);
      #else
      m_add_violations(addr, curr_step, WRITE, read2, READ);
      #endif
      //report race
    }
    
    addr_vec->setWrite1(curr_step);
    //pthread_mutex_unlock(&metadata_mutex);
    return;
  }
  else{
    if (read1 == 0) {  
      addr_vec->setRead1(curr_step);    
      //pthread_mutex_unlock(&metadata_mutex);
      return;
    } 
    else if (read2 == 0) {
     addr_vec->setRead2(curr_step);    
     //pthread_mutex_unlock(&metadata_mutex);
     return;
    }
    //is this safe or should it be read again?
    bool cur_r1 = m_isparallel(curr_step, read1);
    bool cur_r2 = m_isparallel(curr_step, read2);
    if (!cur_r1 && !cur_r2) {
      addr_vec->setRead12(curr_step,0);
      //addr_vec->setRead1(curr_step);
      //pthread_mutex_unlock(&metadata_mutex);
      return;
    } 
    else if (cur_r1 && cur_r2) {
      GraphType lca12 = m_lca(read1, read2);
      GraphType lca1s = m_lca(read1, curr_step);
      if (m_tgraph_nodes[lca1s].m_depth < m_tgraph_nodes[lca12].m_depth )
        addr_vec->setRead1(curr_step);
        //pthread_mutex_unlock(&metadata_mutex);
        return;
...
/*
//this version is lockfree. use this to skip lca accesses
void
OpenmpRaceDetector::m_instrument_access(uint64_t tid, void* addr, bool type){
  #if BASELINE
  return;
  ...
  size_t saddr = (size_t) addr;
    ...
    stat_count.pt_stat[tid].m_enter_instrument_mem++;
  #endif
  // do not instrument if datarace detector has not started yet
  


  #if DUMP_INST_MEM
    report[tid] << addr << "," << type << std::endl;
  #endif 
  ...
  //std::cout << "curr_step = " << curr_step << ", addr_vec = " << addr_vec << ", add_vec->write1 = " << addr_vec->write1 << std::endl;
  if(m_isparallel(curr_step, write1)){
    //report race
    m_add_violations(addr, curr_step, type, write1, WRITE);  
  }
  ...
  if(type == WRITE){
    ...
    //  GraphType lca1s = m_lca(read1, curr_step);
    //  if (m_tgraph_nodes[lca1s].m_depth < m_tgraph_nodes[lca12].m_depth )
    //    //why is this read1 or it doesn't matter
    //    addr_vec->setRead1(curr_step);
	  //    //addr_vec->read1 = curr_step;
    //    
    //    return;
    //}
    //
...
//uncomment to use instrument mem with locks instead of atomics
/*
void
OpenmpRaceDetector::m_instrument_access(uint64_t tid, void* addr, bool type){
  #if BASELINE
  ...
  size_t saddr = (size_t) addr;
    ...
    int tid = omp_get_thread_num();
    stat_count.pt_stat[tid].m_enter_instrument_mem++;
    //stat_count.m_enter_instrument_mem[tid]++;
  #endif
  // do not instrument if datarace detector has not started yet
  if (!omprace_in_parallel.load())
    return;
  //Todo handle serial access and already reported races
  //if(false){
  //  return;
  //}
  ...
  if(primary_ptr == NULL){
    ...
    //if(__sync_bool_compare_and_swap_8(&primary_ptr, NULL, (long*) new_primary_ptr)){//first thread will eval to true
    if(cas_gcc<Metadata*>(&primary_ptr, 0, new_primary_ptr)){//first thread will eval to true
      ...
      #if STAT
      stat_count.pt_stat[tid].m_shadow_mem_buckets++;
      //stat_count.m_shadow_mem_buckets[tid]++;
      #endif
    }
    else{//others should unmap otherwise we'll have memory leak
      munmap(primary_ptr, sec_length);
...
//  GraphType curr_step = m_get_curr_step();
  ...
  pthread_mutex_lock(&addr_vec->metadata_lock);
  //pthread_spin_lock(&addr_vec->metadata_lock);
  
  //std::cout << "curr_step = " << curr_step << ", addr_vec = " << addr_vec << ", add_vec->write1 = " << addr_vec->write1 << std::endl;
  if(m_isparallel(curr_step, addr_vec->write1)){
    //report race
    m_add_violations(addr, curr_step, type, addr_vec->write1, WRITE);  
  }

  if(type == WRITE){
    if(m_isparallel(curr_step, addr_vec->read1)){

      m_add_violations(addr, curr_step, WRITE, addr_vec->read1, READ);
      //report race
    }
    if(m_isparallel(curr_step, addr_vec->read2)){
      m_add_violations(addr, curr_step, WRITE, addr_vec->read2, READ);
      //report race
    }
    //pthread_mutex_lock(&addr_vec->metadata_lock);
    addr_vec->write1 = curr_step;
    pthread_mutex_unlock(&addr_vec->metadata_lock);
    return;
  }
  else{
    if (addr_vec->read1 == 0) {
      ...
      addr_vec->read1 = curr_step;
      pthread_mutex_unlock(&addr_vec->metadata_lock);
      //pthread_spin_unlock(&addr_vec->metadata_lock);
      return;
    } 
    else if (addr_vec->read2 == 0) {
     //pthread_mutex_lock(&addr_vec->metadata_lock);
     addr_vec->read2 = curr_step;
     pthread_mutex_unlock(&addr_vec->metadata_lock);
     //pthread_spin_unlock(&addr_vec->metadata_lock);
     return;
    }
    bool cur_r1 = m_isparallel(curr_step, addr_vec->read1);
    bool cur_r2 = m_isparallel(curr_step, addr_vec->read2);
    if (!cur_r1 && !cur_r2) {
      //pthread_mutex_lock(&addr_vec->metadata_lock);
      addr_vec->read1 = curr_step;
      addr_vec->read2 = 0; 
      ...
      GraphType lca12 = m_lca(addr_vec->read1, addr_vec->read2);
      GraphType lca1s = m_lca(addr_vec->read1, curr_step);
      if (m_tgraph_nodes[lca1s].m_depth < m_tgraph_nodes[lca12].m_depth )
        //pthread_mutex_lock(&addr_vec->metadata_lock);
	      addr_vec->read1 = curr_step;
        pthread_mutex_unlock(&addr_vec->metadata_lock);        
  ...
  #if STAT
    stat_count.pt_stat[tid].m_exit_instrument_mem++;
    //stat_count.m_exit_instrument_mem[tid]++;
  #endif
  
}
*/

//uncomment to make omprace race print out the source line of the detected datarace
/*
void
OpenmpRaceDetector::m_instrument_access(void* addr, bool type, const std::string& loc){
  #if STAT
    int tid = omp_get_thread_num();
    stat_count.pt_stat[tid].m_enter_instrument_mem++;
    //stat_count.m_enter_instrument_mem[tid]++;
  #endif
  // do not instrument if datarace detector has not started yet
  if (!omprace_in_parallel.load() ||  !omprace_init){
    return;
  }
  ...
  if(primary_ptr == NULL){
    ...
    size_t sec_length = (SS_SEC_TABLE_ENTRIES) * sizeof(struct Metadata);
    primary_ptr = (struct Metadata *)mmap(0, sec_length, PROT_READ| PROT_WRITE,
						MAP_PRIVATE|MAP_ANONYMOUS|MAP_NORESERVE, -1, 0);
    m_primary_trie[primary_index] = primary_ptr;
    #if MEM_RECLAIM
    //std::cout << "inserting into active directories, primary_index = " << primary_index << std::endl;
  ...
  pthread_mutex_lock(&m_primary_trie_lock[primary_index]);

  

#if 0
  // No writes =>  hence no races, I have to update metadata 
  if(addr_vec->write1 == 0){
  ...
  if(m_isparallel(curr_step, addr_vec->write1)){
    //report race
    m_add_violations(addr, curr_step, type, addr_vec->write1, WRITE, loc);  
  }

  if(type == WRITE){
    if(m_isparallel(curr_step, addr_vec->read1)){

      m_add_violations(addr, curr_step, WRITE, addr_vec->read1, READ, loc);
      //report race
    }
    if(m_isparallel(curr_step, addr_vec->read2)){
      m_add_violations(addr, curr_step, WRITE, addr_vec->read2, READ, loc);
      //report race
    }
    addr_vec->write1 = curr_step;
  }
    ...
    if (addr_vec->read1 == 0) {
      addr_vec->read1 = curr_step;
      //pthread_mutex_unlock(&metadata_mutex);
      pthread_mutex_unlock(&m_primary_trie_lock[primary_index]);
      return;
    } 
    else if (addr_vec->read2 == 0) {
     addr_vec->read2 = curr_step;
