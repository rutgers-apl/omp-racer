...
void OmpRaceInstrument::initializeCallbacks(Module &M){
    ...
    //OMPInstrumentFunction = checkSanitizerInterfaceFunction(M.getOrInsertFunction(
    //    "__omprace_instrument_access__", VoidTy, void_ptr_ty, int_ty));
    //llvm 9 
    OMPInstrumentFunction = cast<Function>(M.getOrInsertFunction(
        "__omprace_instrument_access__", VoidTy, void_ptr_ty, int_ty).getCallee());

}

bool OmpRaceInstrument::instrumentLoadOrStore(Instruction *I,
                                            const DataLayout &DL) {
    IRBuilder<> IRB(I);
